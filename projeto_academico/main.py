...
Desenvolvido para: Atividade Prática de Machine Learning
===============================================================================
"""
...
try:
    from src.web_scraper import WebScraperReal
    from src.data_analyzer import DataAnalyzer
    from src.linear_regression import LinearRegressionModel
    from src.visualizations import DataVisualizer
except ImportError:
    print("AVISO: Módulos src/ não encontrados. Executando modo simplificado.")
...
def main():
    ...
    """
        ...
        try:
        # ETAPA 1: WEB SCRAPING (COLETA BRUTA - SEM TRATAMENTO)
        
        print("\n" + "="*20 + " ETAPA 1: WEB SCRAPING BRUTO " + "="*20)
        scraper = WebScraperReal()
        raw_data = scraper.coletar_dados(num_registros=1000)  # Pelo menos 1000 registros
        
        print(f"Dados brutos coletados: {len(raw_data)} registros")
        
        # PONTO DE CONTROLE - INPUT PARA CONTINUAR
        ...
        print(" Dados salvos em: data/dados_brutos_raspados.csv")
        print()
        ...
        continuar = input(">>> Continuar? (sim/s): ").lower().strip()
        ...
        print("\n CONTINUANDO PARA AS PRÓXIMAS ETAPAS...")
        time.sleep(2)
        
        # ETAPA 2: ANÁLISE DE DADOS
        
        print("\n" + "="*20 + " ETAPA 2: ANÁLISE DE DADOS " + "="*20)
        analyzer = DataAnalyzer()
        clean_data = analyzer.processar_dados(raw_data)
        analyzer.analise_exploratoria(clean_data)
        
        print(f"Dados processados: {len(clean_data)} registros limpos")
        
        # ETAPA 3: REGRESSÃO LINEAR
        
        print("\n" + "="*20 + " ETAPA 3: REGRESSÃO LINEAR " + "="*20)
        model = LinearRegressionModel()
        results = model.treinar_modelo(clean_data)
        ...
        print("\n" + "="*20 + " ETAPA 4: VISUALIZAÇÃO " + "="*20)
        visualizer = DataVisualizer()
        visualizer.criar_visualizacoes(clean_data, results)
